...
object L1N15LabelHourGCount {
  ...
  case class Params(vvd_fp: String = tianyi.project_pt + "/data/raw/video-visit-data.txt.small",
                    ...
                    out_fp: String = tianyi.project_pt + "/data/fs/l1-n15-label-hour-gcnt_6_5.txt",
                    t_wid: Int = 6,
                    w_len: Int = 5)
  ...
  def run(p: Params): Unit = {
    val conf = new SparkConf()
      .setAppName(s"tianyi-final l1-n15-label-hour-gcnt")
    ...
    val fs = ubd.filter(e => (e.wid == p.t_wid - 1) && (e.did == 7) && (e.l1 != 15)).map {
      p =>
        val bias = (23 - p.hid) * 60 + 60 - p.mid
        (p.uid, Array((bias, p.vcnt)))
    }.reduceByKey(_++_).map {
      case (uid: String, arr: Array[(Int, Int)]) =>
        val v = Array.fill[Double](f_len)(0.0)
        arr.foreach {
          case (bias: Int, n: Int) =>
