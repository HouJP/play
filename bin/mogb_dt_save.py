...
def save_outcome(uids_test, ys_test, vs_test, n_test):
	...
	for i in range(n_test):
		fw.write("%s %f " % (uids_test[i], cos_similarity(ys_test[i], vs_test[i])))
		for j in range(10):
			if (j > 0):
				fw.write(",")
			fw.write("%.3f" % vs_test[i][j])
...
# load user ids
# @return 	user ids
def load_uids(mid, uids):
	fp = "%s/data/fs/uid_fs_%s_byweek" % (env['project_pt'], mid) # test
	fr = open(fp, 'r')
	...
	print "[%s] [INFO] load %s done." % (t_now(), fp)
...
def train_predict(train_id, test_id):
	...
	for iter in range(params['n_round']):
		...
		for j in range(10):
			...
			clf = DecisionTreeRegressor(max_depth = params['max_depth'], max_features = params['max_features'], min_samples_leaf = params['min_samples_leaf'])
		...
		if (iter == params['t_round'] - 1):
			save_outcome(uids_test, ys_test, vs_test, n_test)
		# print "[%s] [INFO] mean_cos_similarity(train)=%f" % (time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time.time())), mean_cos_similarity(ys_train, vs_train, n_train))
		# print "[%s] [INFO] mean_cos_similarity(test)=%f" % (time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time.time())), mean_cos_similarity(ys_test, vs_test, n_test))
...
if __name__ == "__main__":
	...
	print "[%s] [INFO] multi-objective gradient boosting regression done\n" % t_now()