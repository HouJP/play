...
object FilterFileGenerator {
  ...
  def run(params: Params) = {
    ...
    val tuple = KoubeiTrain.load(sc, params.input_fp).filter(e => e.month == params.month_id).map {
      e =>
        (e.user_id, e.location_id)
    }.distinct()

    println(s"[INFO] n(user_id, location_id)=${tuple.count()}")

    tuple.map(e => s"${e._1},${e._2}").saveAsTextFile(params.output_fp)
  }
}